...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    private void init(ISuite suite) {
        ...
        ExtentHtmlReporter htmlReporter =
                new ExtentHtmlReporter(Global_VARS.REPORT_PATH
                        + suite.getName()
                        + ".html");
// report attributes
...
// report system info
        ...
        extent.setSystemInfo("Selenium Version",
                seleniumRev);
    ...
    private void processTestResults(ISuiteResult r) throws Exception {
        ...
        String message = null;
// gather results
        Set<ITestResult> passed =
                r.getTestContext().getPassedTests().getAllResults();
        Set<ITestResult> failed =
                r.getTestContext().getFailedTests().getAllResults();
        Set<ITestResult> skipped =
                r.getTestContext().getSkippedTests().getAllResults();
        Set<ITestResult> configs =
                r.getTestContext().getFailedConfigurations().getAllResults();
        Set<ITestResult> tests =
                new HashSet<ITestResult>();
        tests.addAll(passed);
...
// sort results by the Date field
            List<ITestResult> resultList =
                    new LinkedList<ITestResult>(tests);
            ...
            for (ITestResult result : resultList) {
                ...
                for (String group : result.getMethod().getGroups()) {
                    if (!group.isEmpty()) {
                        ...
                        int size =
                                result.getMethod().getTestClass().toString().
                                        split("\\.").length;
                        String testName =
                            ...
                            test.log(Status.INFO,
                                    ...
                                    MediaEntityBuilder.
                                            createScreenCaptureFromPath(
                                                    bitmapDir
                                                            +
                                                            result.getAttribute("testBitmap")).
                                            build());
                        }
                        test.log(Status.INFO,
                                "STACKTRACE"
                                        + getStrackTrace(result));
                    }
                }
            }
        }
