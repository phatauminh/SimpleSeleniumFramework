...
public abstract class PassionTeaCoBasePO<M extends WebElement> {
    ...
    public PassionTeaCoBasePO() throws Exception {
        PageFactory.initElements(CreateDriver.getInstance().
                getDriver(),this);
    }
    ...
    public void navigate(String page) throws Exception {
        WebDriver driver = CreateDriver.getInstance().getDriver();
        BrowserUtils.waitForClickable(By.xpath("//a[contains(text(),'"
                        + page + "')]"),
        ...
        driver.findElement(By.xpath("//a[contains(text(),'"
                ...
                + "')]")).click();
// wait for page title
        BrowserUtils.waitFor(this.getTitle(),
    ...
    public void verifySpan(String pattern,
                           String text)
            throws AssertionError {
        String getText = null;
        WebDriver driver = CreateDriver.getInstance().getDriver();
     ...
     * @throws AssertionError
     */
    public void verifyHeading(String pattern,
                              String text)
            throws AssertionError {
        String getText = null;
        WebDriver driver = CreateDriver.getInstance().getDriver();
        getText = driver.findElement(By.xpath("//h1[contains(text(),'"
                + pattern
                + "')]")).getText();
        assertEquals(getText, text, "Verify Header Text ");
    }
    /**
     ...
     * @throws AssertionError
     */
    public void verifyParagraph(String pattern,
                                String text)
            throws AssertionError {
        String getText = null;
        WebDriver driver = CreateDriver.getInstance().getDriver();
        getText = driver.findElement(By.xpath("//p[contains(text(),'"
                + pattern
                + "')]")).getText();
